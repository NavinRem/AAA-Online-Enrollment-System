import { isPaid, isCancelled, isUnpaid, isEnrollmentActive, calculateStudentStatus } from './statusUtils'
import { parseDate } from './formatUtils'
import { getProgramProfileURL, getParentProfileURL, getStudentProfileURL, getTeacherProfileURL } from './assetHelper'

/**
 * Calculates enrollment statistics for dashboard/list.
 */
export const calculateTotalEnrollment = (regs) => {
  const now = new Date(), today = new Date(now.setHours(0,0,0,0)).getTime()
  return {
    total: regs.length,
 ...
 */
export const enrichEnrollments = (regs, parents = [], students = [], programs = [], sessions = []) => {
  return regs.map((r) => {
    ...
    return {
      ...r,
      parent: parent ? { id: parent.id, name: parent.name, profileURL: parent.profileURL } : null,
      student: student ? { id: student.id, name: student.name, profileURL: student.profileURL } : null,
      program: prog ? { id: prog.id, title: prog.title || prog.name, profileURL: prog.profileURL } : null,
      sessionSchedule: r.sessionSchedule || (sess?.schedule ? `${sess.schedule.day} ${sess.schedule.timeslot}` : 'N/A'),
      ...
      programProfileURL: getProgramProfileURL(prog?.profileURL || r.programProfileURL, prog?.category),
