import { isPaid, isCancelled, isUnpaid, isEnrollmentActive } from './statusUtils'
...
export const enrichEnrollments = (regs = [], parents = [], students = [], programs = [], sessions = []) => {
  return regs.map((r) => {
    ...
    return {
      ...r,
      parent: parent ? { id: parent.id || parent.uid, name: parent.name, profileURL: parent.profileURL } : null,
      student: student ? { id: student.id || student.uid, name: student.name, profileURL: student.profileURL } : null,
      program: prog ? { id: prog.id || prog.uid, title: prog.title || prog.name, profileURL: prog.profileURL } : null,
