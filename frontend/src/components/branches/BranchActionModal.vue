...
<template>
  <AppModal :show="isOpen" :title="modalTitle" :icon="modalIcon" :error="error" :success="success" maxWidth="600px"
    ...
    <div class="relative min-h-[350px]">
      <!-- ADD / EDIT MODE -->
      <form v-if="type === 'add' || type === 'edit'" id="branchActionForm"
        ...
        <div class="grid grid-cols-2 gap-lg">
          <AppInput v-model="localData.name" label="Branch Name" placeholder="e.g. Central Plaza..." required
            :error="errors.name" :shake="shaking.name" @input="clearError('name')" />
          <AppInput v-model="localData.abbr" label="Abbreviation" placeholder="e.g. CP" required :error="errors.abbr"
        ...
        <div class="grid grid-cols-2 gap-lg">
          <AppInput v-model="localData.location" label="Physical Address" placeholder="Street, City, Landmark..."
            required :error="errors.location" :shake="shaking.location" @input="clearError('location')" />
          <AppInput v-model="localData.phone" label="Support Contact" placeholder="+1 (555) 000-0000" required
        ...
        <div class="flex flex-col gap-md p-lg bg-surface-subtle border border-outline-std/50 rounded-md">
          ...
          <div class="flex flex-wrap gap-2.5">
            <button v-for="color in colorOptions" :key="color" type="button"
              ...
              :class="[localData.color === color ? 'border-primary shadow-lg scale-110' : 'border-transparent opacity-60 hover:opacity-100']"
              :style="{ backgroundColor: `var(--color-${color})` }" @click="localData.color = color">
              <img v-if="localData.color === color" :src="getActionIcon('check')" class="w-4 h-4 brightness-0 invert" />
            </button>
          </div>
        </div>
      </form>
      ...
      <div v-else-if="type === 'delete'"
        class="flex flex-col gap-lg animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div class="bg-white border border-outline-std rounded-2xl p-lg flex flex-col gap-lg shadow-sm" v-if="branch">
      ...
      <AppConfirmOverlay :show="showConfirm"
        :title="type === 'delete' ? 'Delete Branch' : (type === 'edit' ? 'Edit Branch' : 'Add Branch')"
        :subtitle="type === 'delete' ? 'This action will permanently erase this branch and its historical data.' : 'Please verify the logistical details and parameters before proceeding.'"
