  ...
  try {
    ...
    terms.forEach((term) => {
      ; (term.offerings || []).forEach((offering) => {
        ...
        const matchesProgram = props.qualifiedProgramIds.some(id =>
...
const checkConflict = (offering) => {
  ...
  const allCurrent = [
    ...assignments.value.filter(a => !pendingRemoves.value.some(r => r.offeringId === a.offeringId)),
    ...pendingAdds.value
  ]
  const conflict = allCurrent.some((a) => {
    return a.offeringId !== offering.offeringId &&
...
<template>
  <div class="flex flex-col gap-6 min-h-[300px]">
    <div v-if="loading" class="flex-1 flex flex-col items-center justify-center py-20 animate-pulse">
    ...
    <template v-else>
      <div class="flex flex-col gap-3">
        ...
        <div v-if="assignments.length > 0 || pendingAdds.length > 0" class="grid grid-cols-1 gap-3">
          <!-- Existing -->
          <div v-for="assign in assignments" :key="assign.offeringId"
            class="flex items-center justify-between p-4 rounded-[1.25rem] bg-white border border-outline-std transition-all shadow-sm group"
            :class="{ 'opacity-50 grayscale border-dashed': pendingRemoves.some(r => r.offeringId === assign.offeringId) }">
            <div class="flex items-center gap-4">
              ...
              <div class="flex flex-col overflow-hidden">
                <div class="flex items-center gap-2">
                  <span class="text-sm font-black text-content-dark truncate">{{ assign.program?.name }}</span>
                  <AppBadge :status="assign.branch?.abbr || 'HQ'" size="xs" :type="assign.branch?.color || 'blue'" />
                  <AppBadge v-if="pendingRemoves.some(r => r.offeringId === assign.offeringId)" status="To be removed"
                    size="xs" type="red" />
                </div>
                <div class="flex items-center gap-1.5 mt-0.5">
                  <span class="text-xs font-bold text-content-muted">{{ assign.schedule?.day }}</span>
                  <span class="text-xs opacity-30">•</span>
                  <span class="text-xs font-black text-primary">{{ assign.schedule?.time }}</span>
                </div>
              </div>
            </div>
            <AppButton v-if="!pendingRemoves.some(r => r.offeringId === assign.offeringId)" variant="ghost" size="xs"
            ...
            <AppButton v-else variant="ghost" size="xs"
          ...
          <!-- Pending Adds -->
          <div v-for="assign in pendingAdds" :key="'pending-' + assign.offeringId"
            ...
            <AppButton variant="ghost" size="xs"
        ...
        <div v-else
          class="flex flex-col items-center justify-center py-12 rounded-[2rem] border-2 border-dashed border-outline-std bg-surface-subtle/20">
      ...
      <div class="flex flex-col gap-3">
        ...
        <AppAlert v-if="error" type="error" closable @close="error = ''" class="mb-2">{{ error }}</AppAlert>

        <div v-if="availableOfferings.length > 0" class="grid grid-cols-1 gap-3">
          <div v-for="offering in availableOfferings" :key="offering.offeringId"
            class="flex items-center justify-between p-4 rounded-[1.25rem] bg-surface-subtle/30 border border-transparent hover:border-primary/20 hover:bg-white transition-all group/card"
            :class="{
              'opacity-60 grayscale-[0.5] bg-outline-std/5 pointer-events-none': checkConflict(offering) === 'overlap',
            ...
            <div class="flex items-center gap-4">
              <div
                ...
                <img :src="offering.program?.profileURL || getImageUrl('dashboard/card-top-program')"
                  class="w-7 h-7 object-contain" />
              </div>
              <div class="flex flex-col overflow-hidden">
                <div class="flex items-center gap-2">
                  <span class="text-sm font-black text-content-dark truncate">{{ offering.program?.name }}</span>
                  <AppBadge :status="offering.branch?.abbr || 'HQ'" size="xs"
                    :type="offering.branch?.color || 'neutral'" />
                  <span v-if="checkConflict(offering) === 'overlap'"
                    class="text-xs font-black text-error bg-error-soft px-2 py-0.5 rounded-full tracking-tighter border border-error/10">Conflict</span>
                  <span v-else-if="checkConflict(offering) === 'invalid'"
                    class="text-xs font-black text-warning bg-warning-soft px-2 py-0.5 rounded-full tracking-tighter border border-warning/10">Missing</span>
                </div>
                <div class="flex items-center gap-1.5 mt-0.5">
                  <span class="text-xs font-bold text-content-muted">{{ offering.schedule?.day }}</span>
                  <span class="text-xs opacity-30">•</span>
                  <span class="text-xs font-black text-primary">{{ offering.schedule?.time }}</span>
                  <span class="text-xs opacity-30">•</span>
                  <span class="text-xs font-bold text-content-muted tracking-tighter">{{ offering.termName }}</span>
                </div>
              </div>
            </div>
            <AppButton variant="primary" size="xs" class="rounded-xl shadow-md shadow-primary/10 font-black px-4"
              @click="handleAssign(offering)" :disabled="!!checkConflict(offering)">
              Assign
            </AppButton>
          </div>
        </div>
        <div v-else
          class="flex flex-col items-center justify-center py-12 rounded-[2rem] border-2 border-dashed border-outline-std bg-surface-subtle/10">
