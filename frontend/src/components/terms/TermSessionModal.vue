...
<template>
  <AppModal :show="isOpen" :title="`Session Management: ${programName}`" maxWidth="1000px" @close="$emit('close')">
    <template #header>
    ...
    <div class="flex flex-col gap-8 py-4">
      ...
      <div class="bg-primary-soft rounded-[2.5rem] p-8 border border-primary/60 relative overflow-hidden">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-8 relative z-10">
          <div class="flex items-center gap-6">
            <div
              class="w-20 h-20 rounded-[2rem] bg-white shadow-xl shadow-primary/5 border border-outline-std p-4 flex items-center justify-center group-hover:scale-105 transition-transform duration-500">
              <img :src="getProgramProfileURL(program?.profileURL, program?.category)"
                class="w-full h-full object-contain" />
            </div>
            <div class="flex flex-col">
              <div class="flex items-center gap-3">
                <AppBadge v-if="activeBranch" :status="activeBranch.name" size="md" :type="activeBranch.color" />
              ...
              <div class="flex items-center gap-4 mt-3">
                <div class="flex items-center gap-2">
                  <AppBadge :status="schedule?.day" size="md" type="blue" />
                </div>
                <div class="flex items-center gap-2">
                  <span class="text-sm font-bold text-content-dark">{{ schedule?.time }}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div
            ...
            <div class="flex flex-col items-center px-4 border-r border-outline-std/50">
              <span class="text-[10px] font-black text-content-muted uppercase tracking-widest mb-1">Total
                Sessions</span>
              <span class="text-xl font-black text-content-dark">{{ term.totalSessions }}</span>
            </div>
            <div class="flex flex-col items-center pr-4">
              <span class="text-[10px] font-black text-content-muted uppercase tracking-widest mb-2">Term
                Progress</span>
              <div class="flex gap-1.5">
                <div v-for="i in term.totalSessions" :key="i" class="w-4 h-2 rounded-full transition-all duration-700"
                  :class="i <= termProgress ? 'bg-primary shadow-[0_0_10px_rgba(var(--color-primary-rgb),0.5)]' : 'bg-outline-std/40'">
                </div>
              </div>
              <span class="text-[9px] font-black text-primary mt-2 uppercase tracking-tighter">{{ termProgress }} of {{
                term.totalSessions }} Sessions Completed</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="flex items-center justify-between px-2 mt-4">
        <div class="flex flex-col">
          <h4 class="text-sm font-black text-content-dark uppercase tracking-wider">Session Assignments</h4>
          <p class="text-xs font-bold text-content-muted mt-1">Showing only specialists for <span
              class="text-primary font-black">{{ programName }}</span></p>
        </div>
        <div class="flex items-center gap-3">
          <div class="flex -space-x-2">
            <img v-for="t in filteredTeachers.slice(0, 3)" :key="t.id"
              :src="t.profileURL || getImageUrl('profiles/avatar-teacher-man')"
              class="w-7 h-7 rounded-full border-2 border-white shadow-sm" />
            <div v-if="filteredTeachers.length > 3"
              class="w-7 h-7 rounded-full bg-surface-subtle border-2 border-white flex items-center justify-center text-[10px] font-black text-content-muted">
              +{{ filteredTeachers.length - 3 }}
            </div>
          </div>
          <span class="text-[10px] font-black text-content-muted/60 uppercase tracking-widest">Available Experts</span>
        </div>
      </div>
        ...
        <div v-for="i in term.totalSessions" :key="i"
          ...
          <div class="flex items-center justify-between relative z-10">
            <div class="flex items-center gap-2.5">
              <span class="w-2 h-2 rounded-full transition-all duration-500"
                :class="i === termProgress + 1 ? 'bg-primary animate-pulse shadow-[0_0_8px_rgba(var(--color-primary-rgb),0.6)]' : 'bg-outline-std'"></span>
              <span
                class="text-[11px] font-black px-3.5 py-1.5 rounded-lg uppercase tracking-wider shadow-sm transition-colors"
                :class="i <= termProgress ? 'bg-surface-subtle text-content-muted' : 'bg-primary-soft text-primary'">
                Week {{ i }}
              </span>
            </div>
            <div class="flex flex-col items-end">
              <span class="text-[9px] font-black text-content-muted/40 uppercase tracking-widest leading-none">Session
                {{ i
                }}</span>
              <span v-if="i <= termProgress" class="text-[8px] font-black text-green-500/60 uppercase mt-1">Past
                Session</span>
            </div>
          </div>
          ...
          <div class="flex flex-col gap-3 mt-2 relative z-10">
            <div class="flex items-center justify-between ml-1">
              ...
              <span v-if="(currentOffering?.sessionTeachers || [])[i - 1]"
                class="text-[9px] font-black text-green-500 uppercase flex items-center gap-1">
                <span class="w-1 h-1 rounded-full bg-green-500"></span> Assigned
              </span>
              <span v-else-if="responsibleTeachers.length > 0"
                class="text-[9px] font-black text-primary/60 uppercase flex items-center gap-1">
                <span class="w-1 h-1 rounded-full bg-primary/40"></span> Default Specialist
              </span>
            </div>

            <AppSelect :modelValue="(currentOffering?.sessionTeachers || [])[i - 1]?.id || responsibleTeachers[0]?.id"
              :items="filteredTeachers" placeholder="Select Specialist..." size="lg"
              ...
              <template #selected="{ item: t }">
                <div v-if="t" class="flex items-center gap-3 py-1">
                  <div class="relative">
                    <img :src="t.profileURL || getImageUrl('profiles/avatar-teacher-man')"
                      class="w-7 h-7 rounded-full border-2 border-white shadow-sm" />
                    <div
                  ...
                  <div class="flex flex-col">
                    <span class="text-xs font-black text-content-dark truncate max-w-[140px] leading-tight">{{ t.name
                    }}</span>
                    <span class="text-[9px] font-bold text-primary uppercase tracking-tighter">{{ t.branchAbbr || 'HQ'
                    }} Specialist</span>
                  </div>
                </div>
                ...
                <div class="flex items-center gap-4 py-1">
                  ...
                  <div class="flex flex-col">
                    ...
                    <div class="flex items-center gap-1.5 mt-0.5">
                      <span
                        class="text-[10px] font-bold text-content-muted uppercase tracking-wider bg-surface-subtle px-2 py-0.5 rounded">{{
      ...
      <div class="flex items-center justify-between w-full px-2">
        <div class="flex items-center gap-4 text-content-muted">
          <div class="flex items-center gap-2">
            ...
            <span class="text-[10px] font-black uppercase tracking-wider">Auto-Saved</span>
          </div>
          <span class="text-xs font-bold italic opacity-60">Faculty list is restricted to specialists for this
            program.</span>
        </div>
        <div class="flex items-center gap-3">
          ...
          <AppButton variant="primary" size="md" class="px-8 font-black shadow-lg shadow-primary/20"
            @click="$emit('close')">
            Finish
          </AppButton>
        </div>
