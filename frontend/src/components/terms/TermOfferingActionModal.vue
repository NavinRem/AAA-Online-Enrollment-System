...
<template>
  <AppModal :show="isOpen" title="Add Classes to Term" :icon="getActionIcon('plus')" :error="error" :success="success"
    maxWidth="550px" @close="$emit('close')">
        ...
        <AppSelect v-model="form.branchIds" :items="branches" placeholder="Choose branches..." required multiple
          ...
          <template #selected="{ items }">
            ...
            <div v-else class="flex flex-wrap gap-1 py-1">
              <AppBadge v-for="item in items" :key="item.id" :status="item.abbr" :type="item.color || 'blue'" />
            </div>
          </template>
          <template #item="{ item }">
