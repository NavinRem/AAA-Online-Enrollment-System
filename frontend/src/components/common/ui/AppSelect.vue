...
<template>
  <div class="flex flex-col gap-xs text-left w-full" :class="{ 'animate-shake': shake }">
    ...
    <div class="relative w-full" ref="triggerRef">
      <div
        class="relative border-2 border-outline-std rounded-sm bg-surface-subtle cursor-pointer transition-all min-h-11 flex items-center group"
        :class="{
        ...
        >
          ...
          <ul class="list-none p-0 m-0 overflow-y-auto scrollable-v" style="max-height: 220px">
            <li
              ...
              :key="item.id"
              class="px-md py-sm flex items-center gap-sm cursor-pointer transition-colors hover:bg-primary/30 hover:text-black group/item"
              :class="{
