    ...
    <table class="w-full text-left border-collapse">
      <thead>
        <tr class="bg-gray-50/50">
          ...
          <th v-if="showChild" class=" p-md text-xs font-semibold text-content-muted uppercase tracking-widest">Child</th>
          ...
          <th class=" p-md text-xs font-semibold text-content-muted uppercase tracking-widest">Term</th>
          <th v-if="showSchedule" class=" p-md text-xs font-semibold text-content-muted uppercase tracking-widest">Schedule</th>
          <th v-if="showDate" class=" p-md text-xs font-semibold text-content-muted uppercase tracking-widest">Date</th>
      ...
      <tbody class="divide-y divide-gray-50">
        <tr v-for="(item, idx) in items" :key="item.id || idx" class="hover:bg-gray-50/50 transition-colors">
          ...
          <td v-if="showChild" class=" p-md text-sm font-semibold text-black">{{ item.studentName }}</td>
          <td class=" p-md text-sm font-semibold text-black">{{ item.programName }}</td>
          <td class=" p-md">
            <AppBadge :status="item.branchAbbr" :type="item.branchColor" />
          </td>
          <td class=" p-md text-xs font-semibold text-black uppercase tracking-widest">{{ item.termName }}</td>
          <td v-if="showSchedule" class=" p-md text-xs font-semibold text-black leading-tight">{{ item.classSchedule }}</td>
          <td v-if="showDate" class=" p-md text-xs font-semibold text-black tabular-nums">{{ formatDate(item.enrollAt) }}</td>
