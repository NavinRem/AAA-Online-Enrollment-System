...
<template>
  <div
    ...
    <h3 v-if="title" class="text-xl font-bold text-content-dark tracking-tighter mb-6 text-center lg:text-left">
      {{ title }}
    </h3>

    <div v-if="hasAvatar"
      class="w-24 h-24 mx-auto mb-6 rounded-full border-2 border-primary-soft bg-surface-subtle p-1 shadow-sm">
      <img :src="avatarUrl" :alt="title" class="w-full h-full rounded-full object-cover block" @error="
        (e) =>
        (e.target.src =
    ...
    <div class="bg-primary-soft rounded-lg p-5 flex-grow border border-outline-std/10">
      ...
      <div v-else class="text-content-muted italic text-center animate-pulse py-4 font-bold text-2xs ">
        Hydrating Profile...
      </div>
    </div>
  </div>
