...
<template>
  <div
    class="flex items-center justify-between px-2xl py-lg bg-surface-subtle/30 border-t border-outline-std w-full group">
    <div class="flex items-center gap-xs text-sm text-content-muted font-medium">
      <span class="opacity-60 italic">Showing</span>
      <span class="font-black text-content-dark bg-white px-2 py-0.5 rounded-sm shadow-sm ring-1 ring-black/5">{{
        startRange }}</span>
    ...
    <div class="flex items-center gap-2">
      ...
      <button class="pagination-nav-btn group/btn" :disabled="!canPrev" @click="changePage(currentPage - 1)"
        title="Previous Page">
        <img :src="prevIcon" alt="Previous" class="w-5 h-5 transition-transform group-hover/btn:-translate-x-0.5">
      </button>
      ...
      <div class="flex items-center gap-1.5 px-1">
        <template v-for="(page, idx) in pages" :key="idx">
          ...
          <span v-else class="w-8 text-center text-content-muted/40 font-black tracking-widest select-none pt-1">
            ...
          </span>
        </template>
      </div>
      ...
      <button class="pagination-nav-btn group/btn" :disabled="!canNext" @click="changePage(currentPage + 1)"
        title="Next Page">
        <img :src="nextIcon" alt="Next" class="w-5 h-5 transition-transform group-hover/btn:translate-x-0.5">
      </button>
    </div>
  </div>
</template>
...
<style scoped>
.pagination-nav-btn {
  @apply w-10 h-10 flex items-center justify-center rounded-xl bg-white border border-outline-std text-content-muted transition-all duration-200 hover:border-primary hover:text-primary hover:bg-primary-soft/50 disabled:opacity-20 disabled:cursor-not-allowed disabled:hover:bg-white disabled:hover:border-outline-std shadow-sm;
}
</style>
