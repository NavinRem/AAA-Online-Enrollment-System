    ...
    <form id="enrollmentActionForm" @submit.prevent="requestConfirm" novalidate>
      ...
      <div v-if="type === 'pay'" class="flex flex-col gap-lg">
        <div class="bg-white border border-outline-std rounded-std p-lg flex flex-col gap-lg shadow-sm"
          ...
          <div class="grid grid-cols-2 gap-x-lg gap-y-md">
            ...
            <div class="flex flex-col gap-xs">
              <span class="text-2xs font-semibold text-content-muted tracking-wider opacity-60">Parent Registry</span>
              <div class="enroll-identity-row bg-primary-soft/40 border-primary/10">
                <img :src="displaySummary.parentAvatar" class="w-8 h-8 rounded-full border-2 border-white shadow-sm" />
            ...
            <!-- Student -->
            <div class="flex flex-col gap-xs">
              <span class="text-2xs font-semibold text-content-muted tracking-wider opacity-60">Student Name</span>
              <div class="enroll-identity-row bg-primary-soft/40 border-primary/10">
                <img :src="displaySummary.studentAvatar" class="w-8 h-8 rounded-full border-2 border-white shadow-sm" />
                <span class="text-sm font-bold text-content-dark tracking-tight">{{
            ...
            <!-- Program -->
            <div class="flex flex-col gap-xs">
              <span class="text-2xs font-semibold text-content-muted tracking-wider opacity-60">Program Selection</span>
            ...
            <!-- Class & Branch -->
            <div class="flex flex-col gap-xs">
              <span class="text-2xs font-semibold text-content-muted tracking-wider opacity-60">Class and Branch</span>
              <div
                class="enroll-identity-row bg-surface-subtle/30 border-outline-std/20 flex-col !items-start gap-1 p-3">
                <div class="flex items-center justify-between w-full">
                  <span class="text-xs font-semibold">{{ displaySummary.scheduleDay }} ({{
                    displaySummary.scheduleTime
                  }})</span>
          ...
          <div
            ...
            <div class="text-white text-right">
              <span class="text-3xl font-black tracking-tighter">${{ formatPrice(displaySummary.amount) }}</span>
            </div>
          </div>
        </div>
        ...
        <AppAlert type="warning" class="mt-md">
          <div class="flex flex-col gap-0.5">
            <strong class="text-sm font-semibold tracking-tight">Final Verification Required</strong>
            <span class="text-xs opacity-90 font-medium">By confirming, you verify that the payment proof matches the
              tuition amount. This
        ...
        <div class="flex flex-col gap-xs mt-lg">
          ...
          <div class="grid grid-cols-2 gap-sm mt-1">
            ...
            <button type="button" class="enroll-channel-btn" :class="localData.paymentMethod === 'cash'
        ...
        <div class="grid grid-cols-2 gap-lg mt-md">
          <AppSelect v-if="localData.paymentMethod === 'online'" v-model="localData.bankName" :items="['ABA', 'Wing', 'ACLEDA', 'Canadia', 'Sathapana', 'Other'].map((b) => ({
            id: b,
            name: b,
          }))
        ...
        <AppInput v-model="localData.remark" type="textarea" label="Internal Processing Remarks"
          placeholder="Add any specific notes for audit trailing..." :error="errors.remark" :shake="shaking.remark"
      ...
      <div v-if="type === 'cancel'" class="flex flex-col gap-lg">
        ...
        <div class="flex flex-col gap-xs">
          ...
          <AppInput v-model="localData.reason" type="textarea" label="Cancellation Logic / Reason" required
            :error="errors.reason" :shake="shaking.reason" placeholder="Provide a detailed cancel reason..." @input="
              activePreset = '';
            clearError('reason');
            " />
        </div>
      ...
      <div v-if="type === 'delete'" class="flex flex-col gap-lg">
        ...
        <div class="bg-white border border-outline-std rounded-std p-lg flex flex-col gap-lg shadow-sm"
          ...
          <div class="grid grid-cols-2 gap-x-lg gap-y-md">
            <!-- Parent -->
            <div class="flex flex-col gap-xs">
              <span class="text-2xs font-semibold text-content-muted tracking-wider opacity-60">Parent Name</span>
            ...
            <div class="flex flex-col gap-xs">
              ...
              <div
                class="enroll-identity-row bg-surface-subtle/30 border-outline-std/20 flex-col !items-start gap-1 p-3">
                <div class="flex items-center justify-between w-full">
                  <span class="text-sm font-bold text-content-dark">{{ displaySummary.className }}</span>
                  <AppBadge :status="displaySummary.branchAbbr" :type="displaySummary.branchColor" />
                </div>
                <div class="flex items-center justify-between w-full opacity-70">
                  <span class="text-xs font-semibold">{{ displaySummary.scheduleDay }} ({{ displaySummary.scheduleTime
                  }})</span>
                </div>
              </div>
            </div>
