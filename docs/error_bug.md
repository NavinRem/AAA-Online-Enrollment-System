...
- [x] **Class Capacity Desync:**
    ...
    - **Fix:** Implement a Firebase Cloud Function `onWrite` trigger for the `enrollments` collection to automatically re-calculate `currentCount`. (Implemented in `backend/index.js`)

- [x] **Ghost Enrollment Counts on Deletion:**
    ...
    - **Fix:** Update `deleteStudent` to trigger `syncStudentCount` for all affected classes. (Implemented in `studentService.js`)

- [x] **Transaction Limits on Mirror Sync:**
    ...
    - **Fix:** Use a recursive chunked batching utility (already exists in some services but not all). (Implemented `firestoreHelper.js` and refactored all services)

## 🟡 Medium Priority (Logic & UX)

- [x] **Timezone "Off-by-One" Errors:**
    ...
    - **Fix:** Standardize on date-only strings (YYYY-MM-DD) for birthdays or use a consistent timezone library (Luxon/dayjs) on the frontend. (Implemented date-only storage in `studentService.js` and `profileHelper.js`)

- [x] **Inconsistent Term Status Calculation:**
    ...
    - **Fix:** Use a centralized server timestamp for status calculations. (Refactored to use date-string comparisons via `dateHelper.js`)

- [x] **Soft Delete Confusion:**
    ...
    - **Fix:** Standardize on one deletion strategy per collection and document it. (Standardized on `isDeleted: true` across Students, Parents, and Enrollments)

## 🟢 Low Priority (Performance & Cleanup)

- [x] **Large Dataset Performance (Frontend):**
    ...
    - **Fix:** Implement server-side pagination and filtering in `enrollmentService.js`. (Implemented in backend and updated `Enrollments.vue`)

- [x] **Redundant Firestore Queries:**
    ...
    - **Fix:** Use a global state management store (Pinia/Vuex) with cached getters. (Created `dataStore.js` and integrated into the Enrollment module)
