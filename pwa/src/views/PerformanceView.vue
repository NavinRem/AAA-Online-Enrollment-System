...
const displayRecords = computed(() => {
  ...
  return performanceList.value.filter(
    (r) => r.classId === selectedClassId.value || r.id === selectedClassId.value,
  )
})
</script>
...
<template>
  <div class="space-y-5 pb-28">
    ...
    <div
      class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-4.5 rounded-2xl border border-[#e2e8f0] shadow-sm"
    >
      <div>
        <div class="flex items-center gap-2">
          <h1 class="text-lg font-extrabold text-[#0f172a]">Academic Report Card & Exams</h1>
          <span
            class="px-2.5 py-0.5 bg-[#f0f9ff] text-[#0284c7] border border-[#0ea5e9]/20 rounded-full text-[10px] font-black uppercase tracking-wider shadow-2xs"
          >
            {{ performanceList.length }} {{ performanceList.length === 1 ? 'Program' : 'Programs' }}
          </span>
        </div>
        <p class="text-xs text-[#64748b] mt-0.5">
          Official evaluation & exam progress for
          <strong class="text-[#0ea5e9]">{{
    ...
    <div
      v-if="loading"
      class="py-16 text-center text-xs font-bold text-[#64748b] flex flex-col items-center justify-center gap-3"
    >
    ...
    <div v-else class="space-y-6">
      ...
      >
        <!-- Top accent gradient -->
        <div
          class="absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-[#0ea5e9] via-[#38bdf8] to-[#0284c7]"
        ></div>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-1">
          <div class="flex items-start gap-3.5">
            <div
              class="w-11 h-11 rounded-2xl bg-gradient-to-tr from-[#0ea5e9] to-[#0284c7] text-white flex items-center justify-center font-black text-base shadow-md flex-shrink-0"
            >
              ...
              <div class="flex items-center gap-2 flex-wrap">
                ...
                <span
                  :class="[
                    ...
                    rec.branchId &&
                    (String(rec.branchId).toUpperCase() === 'FM' ||
                      String(rec.branchId).toUpperCase() === 'SEN SOK')
                ...
                <!-- Status Badge -->
                <span
                  class="inline-flex items-center justify-center px-3 py-1 rounded-full text-xs font-bold leading-none whitespace-nowrap shadow-2xs bg-[#d1fae5] text-[#064e3b]"
                >
                  {{ rec.isEnrollmentSummary ? 'Active Standing' : 'Verified Report' }}
                </span>
              </div>
              <h3 class="text-base font-black text-[#0f172a] mt-1 leading-snug">
                {{ rec.className || rec.programName || 'Enrolled Class' }}
              </h3>
              <p
                class="text-xs font-extrabold text-[#334155] mt-0.5 flex items-center gap-2 flex-wrap"
              >
                <span
                  >👨‍🏫 Teacher: <strong>{{ rec.instructor || 'Faculty' }}</strong></span
                >
          ...
          <div class="text-right flex-shrink-0">
            <span class="text-xs font-black uppercase tracking-wider text-[#0ea5e9] block">{{
              rec.termName || 'Current Term'
            }}</span>
            <span class="text-[11px] text-[#64748b] font-bold">{{
              rec.evaluationDate ? new Date(rec.evaluationDate).toLocaleDateString() : 'Active Term'
            }}</span>
          </div>
        </div>
        ...
        <div
          class="p-5 rounded-2xl bg-gradient-to-r from-[#0ea5e9] to-[#0284c7] text-white shadow-md relative overflow-hidden flex flex-col sm:flex-row sm:items-center justify-between gap-4"
        >
          <div class="space-y-1 relative z-10">
            ...
            <p class="text-xs text-[#f0f9ff]/90 font-medium">
              Comprehensive performance indicator based on session participation and skill
              check-ins.
            </p>
          </div>
          <div
            class="w-14 h-14 rounded-2xl bg-white/20 backdrop-blur-md flex items-center justify-center text-2xl font-black self-end sm:self-center shadow-inner"
          >
            ★
          </div>
        </div>
        ...
        <div class="p-4.5 rounded-2xl bg-[#f0f9ff] border border-[#0ea5e9]/30 shadow-sm relative">
          <div
            class="flex items-center gap-2 text-[#0284c7] text-xs font-extrabold uppercase tracking-wider mb-2"
          >
          ...
          <blockquote
            class="text-xs sm:text-sm text-[#0f172a] italic leading-relaxed pl-3 border-l-2 border-[#0ea5e9]"
          >
            "{{
              rec.teacherRemarks ||
        ...
        <div class="p-4.5 rounded-2xl bg-[#f8fafc] border border-[#e2e8f0] shadow-2xs space-y-3">
          <h4
            class="text-xs font-extrabold text-[#0f172a] uppercase tracking-wider flex items-center gap-2"
          >
            <span>🎯 Skills & Competencies Tracked</span>
            <span class="text-[11px] text-[#0284c7] font-bold"
              >({{ (rec.skillsMastered || []).length }})</span
            >
          </h4>
          ...
          <div v-if="(rec.skillsMastered || []).length > 0" class="flex flex-wrap gap-2 pt-1">
            ...
            >
              <span
                class="w-4 h-4 rounded-full bg-emerald-600 text-white flex items-center justify-center text-[9px] font-black"
                >✓</span
              >
              <span>{{ skill }}</span>
            </div>
        ...
        <div
          v-if="(rec.grades || rec.examScores || rec.evaluations || []).length > 0"
          class="bg-white rounded-2xl border border-[#e2e8f0] p-4.5 shadow-2xs space-y-3"
        >
          <h4 class="text-xs font-extrabold text-[#0f172a] uppercase tracking-wider">
            ...
            >
              <div>
                <span class="text-sm font-extrabold text-[#0f172a]">{{
                  exam.assessmentName || exam.title || exam.name || 'Assessment'
                }}</span>
                <p v-if="exam.remarks || exam.notes" class="text-xs text-[#334155] mt-0.5">
                  {{ exam.remarks || exam.notes }}
                </p>
              </div>
              <div class="flex items-center gap-2 self-end sm:self-center">
                ...
                <span
                  class="px-3 py-1 rounded-lg bg-[#f0f9ff] border border-[#0ea5e9]/30 text-[#0284c7] text-xs font-extrabold"
                >
                  Grade: {{ exam.grade || 'A' }}
                </span>
              </div>
      ...
      <div
        ...
        class="p-8 bg-white rounded-3xl border border-dashed border-[#e2e8f0] text-center shadow-sm"
      >
        <div
        ...
        <p class="text-xs text-[#64748b] mt-1">
          Once your child enrolls in classes and instructors complete term check-ins, performance
