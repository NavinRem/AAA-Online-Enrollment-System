...
const parentAvatarUrl = computed(() => getParentProfileURL(parentProfileData.value?.profileURL || currentUser.value?.photoURL))
const studentAvatarUrl = computed(() => getStudentProfileURL(studentStore.selectedStudent?.profileURL))
...
const processedClasses = computed(() => {
  ...
  return availableClasses.value.map((item) => {
    ...
    isDuplicate = studentActiveEnrollments.value.some((e) => {
      ...
      if (item.classId && eClassId && item.classId === eClassId && item.termId === eTermId) return true
      if (item.termId && eTermId && item.termId === eTermId) {
        if ((pId && eProgramId && pId === eProgramId) || (pName && eProgramName && pName === eProgramName)) return true
      }
      return false
    })
...
const handleConfirmSelfEnroll = async () => {
  ...
  try {
    const payload = {
      ...
      programName: selectedOffering.value.programName || selectedOffering.value.program?.name || selectedOffering.value.name || '',
    ...
    const enrollmentDoc = res.enrollment || res || {
      ...
      branchObj: selectedOffering.value.branchObj || { abbr: selectedOffering.value.branchId, color: selectedOffering.value.branchColor || 'blue' },
...
<template>
  <div class="space-y-6 pb-36">
    ...
    <div class="bg-white p-6 rounded-3xl border border-[#e2e8f0] shadow-sm space-y-4">
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p class="text-xs text-[#64748b] mt-0.5">
            Select class schedule for <strong class="text-[#0ea5e9]">{{ studentStore.selectedStudent?.name }}</strong> with automatic conflict prevention
          </p>
        </div>
        ...
        <div class="flex items-center gap-2.5">
          <label class="text-xs font-bold text-[#64748b] flex items-center gap-1.5 flex-shrink-0">
            <svg class="w-4 h-4 text-[#0ea5e9]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
            </svg>
            <span>Branch:</span>
          </label>
          <select
      ...
      <div v-if="studentStore.selectedStudent" class="flex items-center justify-between flex-wrap gap-3 p-3.5 bg-[#f8fafc] rounded-2xl border border-[#e2e8f0]">
        <div class="flex items-center gap-3">
          <div class="flex items-center gap-2">
            <div class="w-9 h-9 rounded-full overflow-hidden ring-2 ring-[#0ea5e9]/20 bg-white shadow-2xs">
              <img :src="parentAvatarUrl" alt="Parent" class="w-full h-full object-cover" />
            </div>
            <div class="text-xs">
              <span class="text-[10px] font-bold text-[#64748b] block leading-none">Parent / Guardian</span>
              <span class="font-extrabold text-[#0f172a]">{{ parentProfileData?.name || currentUser?.displayName || 'Parent' }}</span>
            </div>
          </div>
          <span class="text-[#cbd5e1] font-bold">→</span>
          <div class="flex items-center gap-2">
            <div class="w-9 h-9 rounded-full overflow-hidden ring-2 ring-[#10b981]/20 bg-white shadow-2xs">
              <img :src="studentAvatarUrl" alt="Student" class="w-full h-full object-cover" />
            </div>
            <div class="text-xs">
              <span class="text-[10px] font-bold text-[#64748b] block leading-none">Enrolling Student</span>
              <span class="font-extrabold text-[#0f172a]">{{ studentStore.selectedStudent?.name }}</span>
            </div>
          </div>
        </div>
    ...
    <!-- Alert status -->
    <div v-if="errorMessage" class="p-4 bg-red-50 border border-red-200 rounded-2xl text-xs font-bold text-red-700 flex items-center gap-2 shadow-sm">
    ...
    <div v-if="successMessage" class="p-4 bg-emerald-50 border border-emerald-200 rounded-2xl text-xs font-bold text-emerald-800 flex items-center gap-2 shadow-sm">
    ...
    <div v-if="!studentStore.selectedStudent" class="p-8 bg-white border border-[#e2e8f0] rounded-3xl text-center shadow-sm">
      <div class="w-12 h-12 bg-[#f0f9ff] text-[#0284c7] rounded-full flex items-center justify-center mx-auto mb-3 font-extrabold text-xl">
        👧
      </div>
      <h3 class="text-base font-black text-[#0f172a]">Please select a student above</h3>
      <p class="text-xs text-[#64748b] mt-1">We need to check eligibility and existing schedule conflicts first.</p>
    </div>

    <div v-else-if="loading" class="py-16 text-center bg-white rounded-3xl border border-[#e2e8f0] shadow-sm">
      ...
      <p class="text-sm font-extrabold text-[#0f172a]">Loading available courses and checking schedule conflicts...</p>
    </div>
    ...
    <div v-else class="space-y-6">
      ...
      <div class="flex items-center justify-between px-1">
        <div>
          ...
          <p class="text-xs text-[#64748b]">Showing eligible open classes matching {{ studentStore.selectedStudent.name }}'s requirements</p>
        </div>
        <span class="inline-flex items-center justify-center px-3 py-1 rounded-full text-xs font-bold leading-none whitespace-nowrap bg-[#d1fae5] text-[#064e3b]">
          {{ eligibleClasses.length }} Available
        </span>
      </div>

      <div v-if="eligibleClasses.length === 0" class="p-8 bg-white border border-[#e2e8f0] rounded-3xl text-center shadow-sm">
        <p class="text-sm font-extrabold text-[#0f172a]">No classes available for self-enrollment right now</p>
        <p class="text-xs text-[#64748b] mt-1">This might be due to existing enrollments in all active terms or schedule clashes.</p>
      </div>
        ...
        >
          ...
          <div
            :class="[
              ...
              selectedOffering?.id === item.id ? 'bg-gradient-to-r from-[#0ea5e9] via-[#38bdf8] to-[#0284c7] opacity-100' : 'bg-gradient-to-r from-[#0ea5e9] to-[#0284c7] opacity-0 group-hover:opacity-100'
            ]"
          ></div>
          ...
          <div class="space-y-3.5">
            ...
            <div class="flex items-center justify-between gap-2 pt-1 flex-wrap">
              <div class="flex items-center gap-2 flex-wrap">
                ...
                <span v-if="selectedOffering?.id === item.id" class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-black bg-[#0ea5e9] text-white shadow-2xs">
              ...
              <AppBadge :status="`$${formatPrice(item.amount || item.fee || 150)} / term`" type="green" />
            </div>
            ...
            <div class="flex items-start gap-3.5 pt-1">
              <div class="w-12 h-12 rounded-2xl overflow-hidden bg-[#f0f9ff] ring-2 ring-[#e2e8f0] shadow-md flex items-center justify-center flex-shrink-0 group-hover:scale-105 transition-transform">
                ...
                <span v-else class="text-base font-black text-[#0284c7]">
                  {{ (item.programName || item.program?.name || item.name || 'C').charAt(0).toUpperCase() }}
                </span>
              </div>
              <div class="flex-1 min-w-0">
                <h4 class="text-base font-black text-[#0f172a] leading-snug group-hover:text-[#0284c7] transition-colors truncate">
                ...
                <p class="text-xs text-[#64748b] font-bold mt-0.5 flex items-center gap-1 truncate">
                  <span>👨‍🏫 Teacher:</span>
                  <strong class="text-[#334155] truncate">{{ item.instructor || item.teacherName || 'Faculty' }}</strong>
                </p>
              </div>
            </div>

            <!-- Schedule & Capacity Box -->
            <div class="bg-[#f8fafc] group-hover:bg-white border border-[#e2e8f0] rounded-2xl p-3.5 space-y-2 transition-colors shadow-2xs">
              <div class="flex items-center gap-2 text-xs font-extrabold text-[#0f172a]">
                <svg class="w-4 h-4 text-[#0ea5e9] flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <span>{{ item.schedule || 'Flexible Schedule' }}</span>
              </div>
              <div class="flex items-center justify-between text-[11px] font-bold text-[#64748b] pt-1 border-t border-[#e2e8f0]">
                <span class="flex items-center gap-1.5">
                  <span class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
                  <span>{{ item.seatsAvailable !== null ? item.seatsAvailable + ' seats open' : 'Open Registration' }}</span>
                </span>
                <span class="inline-flex items-center justify-center px-2.5 py-0.5 rounded-full text-[10px] font-bold leading-none whitespace-nowrap bg-[#e0f2fe] text-[#0284c7]">
      ...
      <div v-if="conflictedClasses.length > 0" class="pt-6 border-t border-[#e2e8f0] space-y-4">
        <div class="flex items-center justify-between px-1">
          <div>
            ...
            <p class="text-[11px] text-[#94a3b8]">Classes filtered out to prevent time collisions or double enrollment</p>
          </div>
          <span class="inline-flex items-center justify-center px-3 py-1 rounded-full text-xs font-bold leading-none whitespace-nowrap bg-[#fef3c7] text-[#78350f]">
          ...
          >
            <div>
              <div class="flex items-center justify-between gap-2">
                <span class="text-xs font-black text-[#475569]">{{ item.programName || item.program?.name || item.name }}</span>
                <span
                  :class="[
                    'inline-flex items-center justify-center px-2.5 py-0.5 rounded-full text-[10px] font-bold leading-none whitespace-nowrap',
                    item.isDuplicate ? 'bg-[#f3e8ff] text-[#581c87]' : 'bg-[#fef3c7] text-[#78350f]'
                  ]"
                >
                  {{ item.isDuplicate ? 'Duplicate Program' : 'Schedule Conflict' }}
                </span>
              </div>
              <p class="text-[11px] text-[#64748b] font-bold mt-1">{{ item.schedule }} • {{ item.conflictReason }}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    >
      <div class="flex items-start gap-3.5">
        <div class="w-11 h-11 rounded-2xl bg-gradient-to-tr from-[#0ea5e9] to-[#0284c7] text-white flex items-center justify-center font-black text-lg shadow-md flex-shrink-0 mt-0.5">
          ✓
        </div>
        <div class="flex-1 min-w-0">
          <span class="text-[10px] font-black uppercase tracking-widest text-[#0284c7] block">Step 3: Confirm Seat Reservation</span>
          <p class="text-sm font-extrabold text-[#0f172a] leading-tight mt-0.5 truncate">
            Enrolling <strong class="text-[#0ea5e9]">{{ studentStore.selectedStudent?.name }}</strong> into {{ selectedOffering.programName || selectedOffering.program?.name || selectedOffering.name }}
          </p>
          <div class="text-xs text-[#64748b] font-bold mt-1 flex flex-wrap items-center gap-x-2 gap-y-0.5">
            <span class="text-[#334155]">{{ selectedOffering.schedule }}</span>
            <span class="text-[#94a3b8]">•</span>
            <span>Fee: <strong class="text-emerald-700 font-black">${{ selectedOffering.amount || selectedOffering.fee || 150 }}</strong></span>
          </div>
        </div>
      </div>

      <div class="flex flex-col-reverse sm:flex-row items-stretch sm:items-center gap-2.5 pt-2 border-t border-[#f1f5f9]">
        <button
          type="button"
