...
class UserService {
  ...
  async getAllUsers() {
    ...
    const adminList = admins.docs.map((doc) => ({ uid: doc.id, ...doc.data() }));
  ...
  async updateUser(uid, updateData) {
    ...
    const syncFields = ["name", "email", "phone", "profileURL"];
  ...
  async _syncUserMirrors(
    ...
    const parentInfo = this._getUserSnapshot(uid, userData);
    ...
    const studentsSnap = await db
      ...
      .where("parentId", "==", uid)
      .get();
    studentsSnap.forEach((sDoc) => batch.update(sDoc.ref, { parentInfo }));

    const enrollmentsSnap = await db
      .collection(COLLECTIONS.ENROLLMENT)
      .where("parentId", "==", uid)
      .get();
    enrollmentsSnap.forEach((eDoc) => batch.update(eDoc.ref, { parent }));
