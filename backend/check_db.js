...
async function checkTerms() {
  ...
  termsSnap.forEach(doc => {
    const data = doc.data();
    console.log(`Term ID: ${doc.id}, Name: ${data.name}, Start: ${data.startDate}`);
    if(data.offerings && data.offerings.length > 0) {
      console.log(`  Offerings count: ${data.offerings.length}`);
      data.offerings.forEach((off, idx) => {
         console.log(`    Off ${idx}: classId=${off.classId}, branchId=${off.branchId}, progName=${off.program?.name}`);
      });
    }
  });
  ...
  classesSnap.forEach(doc => {
    const data = doc.data();
    console.log(`Class ID: ${doc.id}, ProgName: ${data.program?.name}, SchedCount: ${data.scheduleIds?.length || 0}`);
  });
}
