...
async function findDuplicateStudents() {
  ...
  for (const s of students) {
    ...
    if (name.includes('yang') || name.includes('guang')) {
      console.log(`  🔍 Found potential match: "${s.name}" (ID: ${s.id}, DOB: ${dob}, Parent: ${parentId})`)
    }
  }
  ...
  for (const [key, group] of duplicateGroups) {
    ...
    console.log(`  📌 "${keep.name}" (DOB: ${keep.dob || 'N/A'}, Parent: ${keep.parentId || 'N/A'})`)
    ...
    toDelete.push(...duplicates.map(dup => ({
      ...dup,
      keepId: keep.id,
    })))
  }
...
async function deleteDuplicates(duplicates) {
  ...
  const parentIds = [...new Set(duplicates.map(d => d.parentId).filter(Boolean))]
  ...
  for (const parentId of parentIds) {
    ...
    const deletedIds = new Set(duplicates.filter(d => d.parentId === parentId).map(d => d.id))
    ...
    if (cleanedChildren.length !== currentChildren.length) {
      ...
      console.log(`  ✅ Cleaned parent ${parentId}: removed ${currentChildren.length - cleanedChildren.length} stale reference(s)`)
    }
  }
...
async function main() {
  ...
  console.log('  Mode:', APPLY ? '🟢 APPLY (changes will be written)' : '🔵 DRY RUN (preview only)')
  console.log('=' .repeat(60))
