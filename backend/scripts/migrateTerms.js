...
async function migrate() {
  console.log(
    'Starting Term Migration: Merging branch-specific terms into unified records...',
  )
  ...
  for (const name in termGroups) {
    ...
    for (const other of others) {
      ...
      console.log(
        `  Merging ${other.id} (${otherBranchIds.join(', ')}) into ${master.id}`,
      )
    ...
    const finalMasterDoc = await db
      .collection(COLLECTIONS.TERM)
      .doc(master.id)
      .get()
    await termService.syncEnrollmentsForTerm(master.id, {
      id: master.id,
      ...finalMasterDoc.data(),
    })
