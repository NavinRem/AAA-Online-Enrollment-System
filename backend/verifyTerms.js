...
async function verifyTerms() {
  ...
  try {
    ...
    snapshot.forEach((doc) => {
      ...
      const calculatedStatus = dateHelper.calculateStatus(
        data.startDate,
        data.endDate,
      )
      const expectedEndDate = dateHelper.calculateEndDate(
        data.startDate,
        data.totalSessions || 11,
      )
      ...
      if (statusMismatch || dateMismatch) {
        ...
        if (statusMismatch)
          console.log(
            `  - Status Mismatch: Stored="${data.status}", Calculated="${calculatedStatus}"`,
          )
        if (dateMismatch)
          console.log(
            `  - EndDate Mismatch: Stored="${data.endDate}", Calculated="${expectedEndDate}"`,
          )
      }
    })
