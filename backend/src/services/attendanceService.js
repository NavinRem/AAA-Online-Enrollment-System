...
class AttendanceService {
  ...
  async recordAttendance(classId, sessionId, studentStatuses) {
    if (!classId || !sessionId) throw new Error('Class ID and Session ID are required')

    const attendanceId = `${classId}_${sessionId}`
    const attendanceRef = db.collection(COLLECTIONS.ATTENDANCE || 'attendances').doc(attendanceId)
  ...
  async getClassAttendance(classId) {
    ...
    const snapshot = await db.collection(COLLECTIONS.ATTENDANCE || 'attendances')
