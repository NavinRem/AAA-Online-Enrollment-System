...
async function runTests() {
  console.log('🚀 STARTING AUTOMATED BACKEND DATA INTEGRITY & CALCULATION TESTS 🚀\n');
  let passCount = 0;
  let failCount = 0;
  ...
  try {
    ...
    // Create Term
    await db.collection(COLLECTIONS.TERM).doc(testTermId).set({
      name: 'Test Term',
    ...
    // Create Students
    await db.collection(COLLECTIONS.STUDENT).doc(testStudent1Id).set({ name: 'Active Paid Student', parentId: 'parent-1' });
    await db.collection(COLLECTIONS.STUDENT).doc(testStudent2Id).set({ name: 'Active Unpaid Student', parentId: 'parent-1' });
    await db.collection(COLLECTIONS.STUDENT).doc(testStudent3Id).set({ name: 'Cancelled Student', parentId: 'parent-1' });
    ...
    // ----------------------------------------------------
    console.log('🧪 Running Test 1: getTerm includes paid and unpaid active students...');
    ...
    const offeringClass = termResult.offerings.find(o => String(o.classId) === String(testClassId));
    const offeringStudentIds = offeringClass ? (offeringClass.studentIds || []) : [];
    ...
    if (hasStudent1 && hasStudent2 && !hasStudent3 && offeringStudentIds.length === 2) {
      console.log('✅ Test 1 Passed! Active paid and unpaid students are mapped, cancelled is excluded.');
      passCount++;
    } else {
    ...
    // ----------------------------------------------------
    console.log('🧪 Running Test 2: getAllTerms aggregates only successful paid revenue...');
    ...
    if (targetTermStats && targetTermStats.revenue === 150) {
      console.log('✅ Test 2 Passed! Revenue for the term aggregates to exactly $150 (unpaid and cancelled excluded).');
      passCount++;
    } else {
      console.error(`❌ Test 2 Failed! Expected term revenue to be 150, but got ${targetTermStats ? targetTermStats.revenue : 'N/A'}`);
      failCount++;
    }
    console.log('');
    ...
    // ----------------------------------------------------
    console.log('🧪 Running Test 3: Changing enrollment program updates capacity counters transactionally...');
    ...
    const progADoc = await db.collection(COLLECTIONS.PROGRAM).doc(testProgramAId).get();
    const progBDoc = await db.collection(COLLECTIONS.PROGRAM).doc(testProgramBId).get();

    const countA = progADoc.data().totalEnrolledCount;
    const countB = progBDoc.data().totalEnrolledCount;

    if (countA === 0 && countB === 1) {
      console.log('✅ Test 3 Passed! Program A count was decremented to 0 and Program B was incremented to 1.');
      passCount++;
    } else {
      console.error(`❌ Test 3 Failed! Expected Program A to be 0 and Program B to be 1, but got Program A: ${countA}, Program B: ${countB}`);
      failCount++;
    }
    console.log('');
    ...
    deleteBatch.delete(db.collection(COLLECTIONS.ENROLLMENT).doc(testEnrollment1Id));
    deleteBatch.delete(db.collection(COLLECTIONS.ENROLLMENT).doc(testEnrollment2Id));
    deleteBatch.delete(db.collection(COLLECTIONS.ENROLLMENT).doc(testEnrollment3Id));
