...
test.describe('Terms Management & Metrics View', () => {
  ...
  test('should render term name and dates correctly with parseDate resiliency', async ({
    page,
  }) => {
    // Validate table header/term name
    await expect(
      page.locator('text=Playwright Test Term').first(),
    ).toBeVisible()
  ...
  test('should perform accurate local calculations on metrics', async ({
    page,
  }) => {
    // Calculations:
  ...
  test('should create a new academic term successfully', async ({ page }) => {
    ...
    // Fill the Term form
    await page.fill(
      'input[placeholder*="T1-2026-Saturday"]',
    ...
    await expect(
      page.locator(
        'text=Please verify the academic schedule and parameters before proceeding.',
      ),
    ).toBeVisible()
    ...
    await Promise.all([
      page.waitForResponse(
        (res) =>
          res.url().includes('/terms') && res.request().method() === 'POST',
      ),
