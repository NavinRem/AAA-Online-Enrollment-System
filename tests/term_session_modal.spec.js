...
test.describe('TermSessionModal Component Logic & Rendering Spec', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.route('**/api/branches**', async (route) => {
      await route.fulfill({
        ...
        contentType: 'application/json',
        body: JSON.stringify([
          { id: branchMainId, name: 'Main Campus', color: 'blue' },
        ]),
      })
    })
  ...
  test('should render session weeks correctly and allow updating teacher', async ({
    page,
  }) => {
          ...
          offerings: [
            {
              ...
              program: {
                ...
                category: { name: 'Math' },
              }, // FIX: Need program to render correctly
    ...
    await page.route(
      `**/api/terms/${termId}/offerings/${offeringId}`,
      async (route) => {
    ...
    const manageFacultyButton = page.locator('button', {
      hasText: 'Manage Faculty',
    })
    ...
    const week2Container = page
      .locator('div.group\\/session')
      .filter({ hasText: 'Week 2' })

    // The dropdown has placeholder "Select Specialist..." or default text "Mr. Smith"
    const dropdownTrigger = week2Container
      .locator('button, .ui-select-trigger')
      .first()
    ...
    await expect(
      week2Container.locator('text=Ms. Johnson').first(),
    ).toBeVisible()
