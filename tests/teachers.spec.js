...
test.describe('Teachers Management & Assignments View', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.route('**/api/teachers**', async (route) => {
      ...
      if (method === 'GET') {
        ...
        method === 'PATCH' &&
        route.request().url().includes('/teachers/teacher-1')
      ) {
  ...
  test('should display metric cards and teachers list correctly', async ({
    page,
  }) => {
    ...
    // Verify stats cards
    await expect(
      page.locator('.ui-metric-card', { hasText: 'Total Teachers' }),
    ).toContainText('2')
    await expect(
      page.locator('.ui-metric-card', { hasText: 'Active Teachers' }),
    ).toContainText('1')
  ...
  test('should filter by active / inactive status successfully', async ({
    page,
  }) => {
    ...
    const inactiveOption = page.locator('.toolbar-filter-option', {
      hasText: 'Inactive',
    })
  ...
  test('should display assigned classes and program linkages', async ({
    page,
  }) => {
    // Wait for page to be stable and mounted
    await expect(page.locator('text=Alice Smith')).toBeVisible()
  ...
  test('should validate form inputs and handle errors on create', async ({
    page,
  }) => {
    // Wait for page to be stable and mounted
    await expect(page.locator('text=Alice Smith')).toBeVisible()
    ...
    await expect(
      page.locator('text=New Teacher Registration').first(),
    ).toBeVisible()
  ...
  test('should add a new teacher successfully with correct payload', async ({
    page,
  }) => {
    // Wait for page to be stable and mounted
    await expect(page.locator('text=Alice Smith')).toBeVisible()
    ...
    const programOption = page
      .locator('.fixed.bg-white.border-2.border-primary li')
      .locator('text="Advanced Robotics"')
      .first()
    ...
    await expect(
      page.locator('text=Verify details before proceeding.'),
    ).toBeVisible()
    ...
    await Promise.all([
      page.waitForResponse(
        (res) =>
          res.url().includes('/teachers') && res.request().method() === 'POST',
      ),
  ...
  test('should edit an existing teacher successfully', async ({ page }) => {
    ...
    const editOption = page
      .locator('.ui-dropdown-item:has-text("Edit")')
      .first()
    await expect(editOption).toBeVisible()
    await editOption.click({ force: true })
    await expect(
      page.locator('text=Edit Teacher Profile').first(),
    ).toBeVisible()
    ...
    await Promise.all([
      page.waitForResponse(
        (res) =>
          res.url().includes('/teachers/teacher-1') &&
