...
test.describe('Program Detail View', () => {
  ...
  test('should render basic program information correctly', async ({ page }) => {
    // Check Title
  ...
  test('should calculate metrics correctly from mocked data', async ({ page }) => {
    // Logic: 2 enrollments of $500 each = $1,000 Total Revenue
    // Unique students = 2
    
    const revenueCard = page.locator('.ui-metric-card', { hasText: 'Total Revenue' });
    await expect(revenueCard).toContainText('$1,000');

    const studentCard = page.locator('.ui-metric-card', { hasText: 'Total Students' });
