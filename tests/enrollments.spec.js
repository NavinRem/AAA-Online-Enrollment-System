...
test.describe('Enrollments Management & Billing View', () => {
  ...
  test('should render the list of enrollments and metrics cards correctly', async ({
    page,
  }) => {
    // Assert Overview Title
    await expect(
      page.locator('h2:has-text("Enrollment Overview")'),
    ).toBeVisible()

    // Verify stats cards are rendered with correct calculated statistics
    await expect(
      page.locator('.ui-metric-card', { hasText: 'Total Enrollment' }),
    ).toContainText('2')
    await expect(
      page.locator('.ui-metric-card', { hasText: 'Total Paid Enrollment' }),
    ).toContainText('1')
    await expect(
      page.locator('.ui-metric-card', { hasText: 'Total Unpaid Enrollment' }),
    ).toContainText('1')
  ...
  test('should filter list items with search searchBox reactively', async ({
    page,
  }) => {
