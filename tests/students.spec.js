...
test.describe('Students Management & Profiles View', () => {
  ...
  test.beforeEach(async ({ page }) => {
    page.on('console', (msg) =>
      console.log('STUDENT TEST BROWSER LOG:', msg.text()),
    )
    page.on('pageerror', (err) =>
      console.error('STUDENT TEST BROWSER ERROR:', err.message),
    )
    ...
    await page.route('**/api/branches**', async (route) => {
      await route.fulfill({
        status: 200,
        body: JSON.stringify([
          { id: 'branch-1', name: 'Main Campus', code: 'MC' },
        ]),
      })
    })
  ...
  test('should compute total and active metrics accurately', async ({
    page,
  }) => {
    // Total Students = 2
    const totalCard = page.locator('.ui-metric-card', {
      hasText: 'Total Students',
    })
    await expect(totalCard).toContainText('2')

    // Enrolled Students = 1 (Jimmy Doe has active status)
    const enrolledCard = page.locator('.ui-metric-card', {
      hasText: 'Active (Studying)',
    })
    await expect(enrolledCard).toContainText('1')

    // Not Enrolled = 1 (Janey Smith has inactive status)
    const notEnrolledCard = page.locator('.ui-metric-card', {
      hasText: 'Inactive (Stopped)',
    })
    await expect(notEnrolledCard).toContainText('1')
  })

  test('should filter student names reactively using search searchBox', async ({
    page,
  }) => {
  ...
  test('should register a new child student linked to a parent successfully', async ({
    page,
  }) => {
    ...
    await expect(
      page.locator('text=Please verify details before proceeding.'),
    ).toBeVisible()
    ...
    await Promise.all([
      page.waitForResponse(
        (res) =>
          res.url().includes('/students') && res.request().method() === 'POST',
      ),
