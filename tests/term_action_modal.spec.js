...
test.describe('TermActionModal Component Rendering & Date Mathematics Spec', () => {
  ...
  test('should accurately calculate date math and handle empty vs populated branches', async ({ page }) => {
    ...
    const globalStartDateInput = page.locator('form > div:not(.border-t) input[type="date"]').first();
    await expect(globalStartDateInput).toBeVisible();

    // 2. Boundary Checklist C: Calculated Data Math (totalSessions watch calculation)
    // Populate Term Name
    const nameInput = page.locator('div:has(> label:has-text("Term Name")) input').first();
    ...
    const calculatedEndDateInput = page.locator('div:has(> label:has-text("Auto-calculated End Date")) input').first();
    await expect(calculatedEndDateInput).toHaveValue('2026-07-10');

    // Change total sessions from 11 to 12
    const sessionsInput = page.locator('div:has(> label:has-text("Total Sessions")) input').first();
    ...
    const branchSchedulingSection = page.locator('text=Branch-Specific Scheduling').first();
