...
test.describe('Component UI Rendering & Calculation Consistency Tests', () => {
        ...
        body: JSON.stringify([
          {
            id: 'cat-1',
            name: 'Robotics',
            profileURL: 'profiles/cat-robotics',
          },
        ]),
      })
    })
  ...
  test('should render DataMetricCard with styling consistency across different modules', async ({
    page,
  }) => {
    // Mock standard data
    ...
    await expect(parentsCard).toHaveClass(
      /ui-metric-card flex flex-col items-center/,
    )
    ...
    await expect(termsCard).toHaveClass(
      /ui-metric-card flex flex-col items-center/,
    )
    await expect(termsCard).toHaveClass(/rounded-std shadow-sm flex-1/)
  })

  test('should render calculated data and font optimizations on the Dashboard', async ({
    page,
  }) => {
    ...
    const weeklyRevenueCard = thisWeekSection.locator(
      '.ui-metric-card:has-text("Payments")',
    )
    ...
    const activeTermPanel = page.locator(
      'span:has-text("Active Academic Term")',
    )
    await expect(activeTermPanel).toBeVisible()

    // Validate initial active term name
    const activeTermName = page
      .locator('.relative.overflow-hidden span:has-text("Summer 2026")')
      .first()
    await expect(activeTermName).toBeVisible()
  })

  test('should render fetched data accurately inside the ParentActionModal', async ({
    page,
  }) => {
    // Mock parents list
    await page.route('**/api/parents**', async (route) => {
