...
test.describe('Trials Management & Bookings View', () => {
          ...
          body: JSON.stringify([
            {
              ...
              student: {
                id: studentId,
                name: 'Alice Smith',
                profileURL: 'profiles/avatar-boy',
              },
              parent: {
                id: parentId,
                name: 'John Smith',
                profileURL: 'profiles/avatar-man',
              },
            },
          ]),
        })
  ...
  test('should display metric cards and trials list correctly', async ({
    page,
  }) => {
    ...
    // Verify stats cards
    await expect(
      page.locator('.ui-metric-card', { hasText: 'Total Trials' }),
    ).toContainText('1')
    await expect(
      page.locator('.ui-metric-card', { hasText: 'Booked Trials' }),
    ).toContainText('1')
    await expect(
      page.locator('.ui-metric-card', { hasText: 'Walk-in Trials' }),
    ).toContainText('0')
    await expect(
      page.locator('.ui-metric-card', { hasText: 'Successful Trials' }),
    ).toContainText('0')
  ...
  test('should filter by booking type reactively', async ({ page }) => {
    ...
    const walkinOption = page.locator('.toolbar-filter-option', {
      hasText: 'Walk-in',
    })
  ...
  test('should book a new guest walk-in trial successfully', async ({
    page,
  }) => {
    // Wait for stability
    await expect(page.locator('text=Alice Smith')).toBeVisible()
    ...
    await expect(
      page.locator('text=Book New Trial Session').first(),
    ).toBeVisible()
    ...
    await page.fill(
      'input[placeholder="e.g. parent@example.com"]',
      'obiwan@jedi.org',
    )
    ...
    await expect(
      page.locator(
        'text=Please review trial details carefully before confirming.',
      ),
    ).toBeVisible()
    ...
    await Promise.all([
      page.waitForResponse(
        (res) =>
          res.url().includes('/trials') && res.request().method() === 'POST',
      ),
